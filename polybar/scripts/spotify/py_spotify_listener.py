...
def event_handler(*args, **kwargs):
    ...
    data = unwrap(args)
    if 'Metadata' not in data[1] or 'mpris:trackid' not in data[1]['Metadata']:
        return
    track_id = data[1]['Metadata']['mpris:trackid']
    if 'spotify' in track_id:
        playback_status = data[1]['PlaybackStatus']
        new_state = (track_id, playback_status)
        if (current_state is None) or (new_state != current_state):
            if playback_status == "Playing":
                ...
                system("polybar-msg hook spotify_playpause 3")
            current_state = new_state


def spotify_owner_changed(new_owner):
    if new_owner == '':
        print('spotify is no longer running')
...
bus.watch_name_owner(bus_name='org.mpris.MediaPlayer2.spotify', 
                     callback=spotify_owner_changed) 
